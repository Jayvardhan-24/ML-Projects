...
class RouterAgent(BaseAgent):
    ...
    def _safe_fallback_response(self, query: str, error: Exception) -> Dict:
        ...
        logger.warning(f"Using heuristic fallback classification for query: {query}")
        import re
        ...
        category_rules = {
            "Technical": [r"\berror\b", r"\bissue\b", r"\bfail|failed|failing\b", r"e\d{3,5}", r"troubleshoot", r"integration", r"api"],
            "Billing": [r"\border\b", r"\binvoice\b", r"\bbilling\b", r"\bcharge|charged|charges\b", r"ord-\d+", r"payment"],
            "Product": [r"\bproduct\b", r"\bfeature|features\b", r"\bpricing|price|cost\b", r"\bplan|plans\b", r"\bcompare|comparison\b", r"annual", r"discount"],
            "Account": [r"\baccount\b", r"acc-\d+", r"\buser\b", r"\badd users|add user|users?\b", r"subscription"],
        }
        ...
        # If multiple categories detected, return multi-part structure
        if len(detected) >= 2:
            parts = [{"query_part": query, "classification": cat} for cat in detected[:3]]
            return {"multi_part": True, "parts": parts, "confidence": 0.7}

        # If single clear category
        if len(detected) == 1:
            return {
...
class ProductSpecialistAgent(BaseAgent):
    ...
    async def process(
        ...
        # Fallback if LLM is unavailable or failed
        if not response or response.startswith("I encountered an error"):
            # Try to craft a direct, actionable response from the catalog and retrieved info
            try:
                catalog_products = self.product_catalog.get("products", [])
                selected = None
                for product in catalog_products:
                    ...
                    if name and name in query.lower():
                        selected = product
                        break
                # Fallback heuristic: pick Pro plan for generic queries mentioning features/pricing
                if not selected and ("pro" in query.lower() or "features" in query.lower()):
                    selected = next((p for p in catalog_products if (p.get("name") or "").lower().startswith("cloud manager pro")), None)

                if selected:
                    price = selected.get("price", {})
                    monthly = price.get("monthly", "N/A")
                    annual = price.get("annual", "N/A")
                    features = selected.get("features", [])
                    feature_lines = []
                    for feat in features:
                        feature_lines.append(f"- {feat.get('name', 'Feature')}: {feat.get('description', '')}")
                    limitations = selected.get("limitations", [])
                    limitations_text = "\n".join([f"- {x}" for x in limitations]) if limitations else "- None listed"
                    response = (
                        f"Here are the key details for {selected.get('name')}:\n\n"
                        f"Pricing: ${monthly}/month or ${annual}/year (10% annual discount).\n\n"
                        f"Key features:\n" + ("\n".join(feature_lines) or "- See docs") + "\n\n"
                        f"Limitations:\n{limitations_text}\n\n"
                        f"If you'd like, I can compare this with other plans or dive deeper into any specific feature."
                    )
                else:
                    # Use retrieved info as fallback text
                    response = (
                        "Based on our documentation, here are relevant details I found:\n\n"
                        f"{relevant_info}\n\n"
                        "Let me know if you want a comparison between products or more pricing details."
                    )
            except Exception as e:
                logger.error(f"Product fallback composition failed: {e}")
                response = (
                    "I can help with product information and comparisons. Could you specify the product or feature you're interested in?"
                )
...
class TechnicalSupportAgent(BaseAgent):
    ...
    async def process(
        ...
        if not response or response.startswith("I encountered an error"):
            try:
                title = (diagnostic_info or {}).get("name", "Technical issue")
                steps = (diagnostic_info or {}).get("solutions", [])
                doc_link = (diagnostic_info or {}).get("documentation_link", "")
                steps_text = "\n".join([f"{i+1}. {s}" for i, s in enumerate(steps)]) if steps else "- Check logs and configuration settings\n- Verify credentials and network access\n- Consult documentation"
                response = (
                    f"I understand you're experiencing: {title}.\n\n"
                    f"Here are actionable steps to resolve it:\n{steps_text}\n\n"
                    f"Additional context from documentation:\n{relevant_info}\n\n"
                    f"Reference: {doc_link}"
                )
            except Exception as e:
                logger.error(f"Technical fallback composition failed: {e}")
                response = (
                    "Let's troubleshoot this step by step: check logs for errors, verify credentials, ensure network connectivity, and consult the relevant section in our docs."
                )
...
class OrderBillingAgent(BaseAgent):
    ...
    async def process(
        ...
        if not response or response.startswith("I encountered an error"):
            try:
                lines: List[str] = []
                lines.append("I'd be happy to help with your billing/order question.")
                if order_id:
                    if order_details:
                        status = order_details.get("status", "unknown").capitalize()
                        total = order_details.get("total", "N/A")
                        order_date = order_details.get("order_date", "N/A")
                        ship_date = order_details.get("shipping_date", "N/A")
                        delivery_date = order_details.get("delivery_date", "N/A")
                        item_names = []
                        for item in order_details.get("items", []):
                            pid = item.get("product_id", "item")
                            qty = item.get("quantity", 1)
                            price = item.get("price", "")
                            item_names.append(f"{pid} ({qty} x ${price})")
                        lines.extend([
                            f"Order {order_id} status: {status}",
                            f"Order Date: {order_date}",
                            f"Shipping Date: {ship_date}",
                            f"Delivery Date: {delivery_date}",
                            f"Items: " + ", ".join(item_names) if item_names else "Items: N/A",
                            f"Total Amount: ${total}",
                        ])
                    else:
                        lines.append(f"I could not find order {order_id}. Please verify the ID.")
                if account_id:
                    if account_details:
                        sub = account_details.get("subscription", {})
                        plan = sub.get("plan", "N/A")
                        status = sub.get("status", "N/A")
                        lines.append(f"Account {account_id} subscription: {plan} ({status}).")
                    else:
                        lines.append(f"I could not retrieve account {account_id}.")
                response = "\n".join(lines)
            except Exception as e:
                logger.error(f"Billing fallback composition failed: {e}")
                response = "Here are the current details I can retrieve about your order/account."
...
class AgentOrchestrator:
    ...
    def add_account_management_agent(self):
        ...
        try:
            ...
            logger.info("Account Management Agent added/enabled")
            return True
        except Exception as e:
